...
import { getNonCollections, insertNonCollection, updateNonCollection, deleteNonCollection } from '../utils/checklistDb';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'insert': {
        const { nonCollection } = req.body;
        if (!nonCollection || typeof nonCollection !== 'object') {
          return res.status(400).json({ success: false, error: 'nonCollection é obrigatório' });
        }
    ...
    console.error('[CHECKLIST_NON_COLLECTIONS] Erro:', error?.message || error);
    return res.status(500).json({ success: false, error: 'Erro ao processar operação de non-collections' });
  }
}
